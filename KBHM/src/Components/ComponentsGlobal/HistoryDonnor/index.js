    ...
    const FeatApiDonnor = async () => {
        ...
        const data = await CheckHistoryDonnor({ IdentityID: Identity }).then(rs => {
            ...
            rs.forEach((data, index) => {
                DataHisApi.push({
                    key: index,
                    label: `Lịch sử lần hiến ngày: ${dayjs(data.dateIn).format('DD/MM/YYYY')} - BCI: ${data.bci}  - ABORH ${data.aborh}  `,
                    children: `Xét nghiệm ${data?.resultBloods.map((rb) => { return `${rb.testCode} : ${rb.result}`; }).join(". - ")}`
                })
            })
            return DataHisApi;
