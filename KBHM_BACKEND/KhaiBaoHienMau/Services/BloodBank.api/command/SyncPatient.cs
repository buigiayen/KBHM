...
namespace BloodBank.api.command
{
    public class SyncPatient : ISyncDonnor
    {
        ...
        public async Task<Services.lib.Sql.HttpObject.APIresult> SyncDonnorEx(Donnor.tbl_Donor donnor)
        {
            ...
            {
                donnor.DonateID = GetDonateID(donnor.BloodSourceLocationId).Result;
            }
            ...
            string ActionDonnor = @$"BEGIN
                                       ...
                                       IF NOT EXISTS (SELECT * FROM tbl_Donor D  WHERE (D.IdentityID =@IdentityID or D.DonorCode = @DonorCode))
                                        ...
                                        insert into tbl_Donor (DateIn,DonorCode, sex,  Age , CCCD,Address,DonorName,DonorNameUnsign,Phone,BirthDay, TypeOf, rowguid , JobID, ContactAddress, ABO, Rh ) values(CAST(GETDATE() AS DATE),@DonorCode,@Sex,@Age,@CCCD,@Address,@DonorName,@DonorNameUnsign,@Phone,@BirthDay,1,@ID,@JobID,@ContactAddress,@ABO,@Rh );
                                         END
                                       ELSE
                                        BEGIN
                                        Print('Update Patient')
                                        update tbl_Donor set DonorCode=@DonorCode, sex=@Sex, Age=@Age, CCCD=@CCCD, Address=@Address,DonorName=@DonorName,DonorNameUnsign=@DonorNameUnsign,Phone=@Phone, BirthDay=@BirthDay, JobID=@JobID, ContactAddress=@ContactAddress, ABO=@ABO, Rh=@Rh where DonorCode=@DonorCode;   
                                      end
        ...
        public async Task<HttpObjectData.APIresult> SyncDonorDelay(Model.BloodDonationDelay delay)
        {
            ...
            delay.LeverDelay = GetLeverDelay(delay.DonorCode).Result == null ? 1 : GetLeverDelay(delay.DonorCode).Result + 1;
            string rowguid = "declare @ID nvarchar(100); set @ID = (select NEWID());";
            ...
            string SQL = $@"BEGIN
                                    --- Xử lý ở Donnor
                                       IF NOT EXISTS (SELECT * FROM tbl_Donor D  WHERE  D.DonorCode = @DonorCode)
                                                       BEGIN
                                         Print('Insert Patient')
                                        insert into tbl_Donor (DateIn,DonorCode, sex,  Age , CCCD,Address,DonorName,DonorNameUnsign,Phone,BirthDay, TypeOf, rowguid , JobID, ContactAddress, ABO, Rh ) values(CAST(GETDATE() AS DATE),@DonorCode,@Sex,@Age,@CCCD,@Address,@DonorName,@DonorNameUnsign,@Phone,@BirthDay,1,@ID,@JobID,@ContactAddress,@ABO_Donor,@Rh_Donor );
                                         END
                                       ELSE
                                        BEGIN
                                        Print('Update Patient')
                                        update tbl_Donor set DonorCode=@DonorCode, sex=@Sex,  Age=@Age, CCCD=@CCCD, Address=@Address,DonorName=@DonorName,DonorNameUnsign=@DonorNameUnsign,Phone=@Phone,BirthDay=@BirthDay, JobID=@JobID , ContactAddress=@ContactAddress, ABO=@ABO_Donor,Rh=@Rh_Donor where  DonorCode=@DonorCode;   
                                      end
		                                BEGIN
