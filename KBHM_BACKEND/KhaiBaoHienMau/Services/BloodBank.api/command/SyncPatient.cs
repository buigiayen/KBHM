...
namespace BloodBank.api.command
{
    public class SyncPatient : ISyncDonnor
    {
        ...
        {
            donnor.SID = string.Format("{0}_{1}", donnor.DateIn.ToString("ddMMyyyy"), donnor.DonorExCode);
									  ...
									  --- Xử lý ở Donnor_ex
									   IF NOT EXISTS (SELECT * FROM tbl_Donor D join tbl_Donor_Examine E on D.DonorID = E.DonorID  WHERE (D.IdentityID =@IdentityID or D.DonorCode = @DonorCode) and E.DonorExCode = @DonorExCode )
                                         ...
                                         Print('Insert Donnor_ex')
											...
											insert tbl_Donor_Examine (DonorExCode, DonorID,BloodSourceLocationId,BloodVolume,ElementID, DateIn, SID) values (@DonorExCode, @DonorID_Insert ,@BloodSourceLocationId,@BloodVolume,@ElementID,@DateIn, @SID);
