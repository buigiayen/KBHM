        ...
        public async Task<Services.lib.Sql.HttpObject.APIresult> SyncDonnorEx(Donnor.tbl_Donor donnor)
        {
            ...
            string ActionDonnor = @$"BEGIN
                                       ...
                                       IF NOT EXISTS (SELECT * FROM tbl_Donor D  WHERE (D.IdentityID =@IdentityID or D.DonorCode = @DonorCode))
                                        ...
                                        insert into tbl_Donor (DateIn,DonorCode, sex,  Age , CCCD,Address,DonorName,DonorNameUnsign,Phone,BirthDay, TypeOf, rowguid , JobID, ContactAddress, ABO, Rh ) values(Getdate(),@DonorCode,@Sex,@Age,@CCCD,@Address,@DonorName,@DonorNameUnsign,@Phone,@BirthDay,1,@ID,@JobID,@ContactAddress,@ABO,@Rh );
                                         END
                                       ELSE
                                        BEGIN
            ...
            string SQL = $@"BEGIN
                                    --- Xử lý ở Donnor
                                       IF NOT EXISTS (SELECT * FROM tbl_Donor D  WHERE (D.IdentityID =@IdentityID or D.DonorCode = @DonorCode))
                                                       BEGIN
                                         Print('Insert Patient')
                                        insert into tbl_Donor (DateIn,DonorCode, sex,  Age , CCCD,Address,DonorName,DonorNameUnsign,Phone,BirthDay, TypeOf, rowguid , JobID, ContactAddress, ABO, Rh ) values(Getdate(),@DonorCode,@Sex,@Age,@CCCD,@Address,@DonorName,@DonorNameUnsign,@Phone,@BirthDay,1,@ID,@JobID,@ContactAddress,@ABO_Donor,@Rh_Donor );
                                         END
                                       ELSE
                                        BEGIN
                                        Print('Update Patient')
